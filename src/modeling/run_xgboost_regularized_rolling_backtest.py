"""Run rolling-origin backtesting for regularized XGBoost on cluster-day features."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="reports/modeling/xgboost_regularized_backtest")
    parser.add_argument("--seed", type=int, default=42)
    return parser


def _build_model(seed: int) -> XGBRegressor:
    """Build a more regularized XGBoost model to reduce overfitting."""
    return XGBRegressor(
        n_estimators=500,
        learning_rate=0.05,
        max_depth=5,  # Reduced from 8
        min_child_weight=10,  # Increased from default 1
        gamma=2.0,  # Penalize splits; was 0
...
def main() -> None:
            ...
            fold_rows.append(
                {
                    ...
                    "model": "xgboost_regularized_cluster_daily",
                    **metrics(y_h, pred_h),
                }
            )
            fold_rows.append(
    ...
    if {"xgboost_regularized_cluster_daily", "seasonal_naive_7d"}.issubset(pivot.columns):
        improvement = (
            (pivot["seasonal_naive_7d"] - pivot["xgboost_regularized_cluster_daily"])
            / pivot["seasonal_naive_7d"]
            * 100
        )
        improvement_df = improvement.rename("xgboost_regularized_mae_improvement_vs_naive_pct").reset_index()
        improvement_df.to_csv(output_dir / "xgboost_regularized_vs_naive_improvement.csv", index=False)

    diagnostics_df = pd.DataFrame(diagnostics_rows)
    diagnostics_df.to_csv(output_dir / "overfitting_diagnostics.csv", index=False)

    print("Rolling backtest complete (regularized XGBoost).")
    print(f"Saved outputs in: {output_dir}")
    print(summary.to_string(index=False))
    
    print("\n=== Overfitting Diagnostics ===")
    print(f"Average train MAE: {diagnostics_df['train_mae'].mean():.2f}")
    print(f"Average eval MAE (max horizon): {diagnostics_df['eval_mae_max_horizon'].mean():.2f}")
    print(f"Average train-eval MAE gap: {diagnostics_df['mae_gap'].mean():.2f}")
