"""Run rolling-origin backtesting for LightGBM on cluster-day features."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="reports/modeling/lightgbm_backtest")
    parser.add_argument("--seed", type=int, default=42)
    return parser


def _build_model(seed: int) -> LGBMRegressor:
...
def main() -> None:
    ...
    if {"lightgbm_cluster_daily", "seasonal_naive_7d"}.issubset(pivot.columns):
        improvement = (
            (pivot["seasonal_naive_7d"] - pivot["lightgbm_cluster_daily"]) / pivot["seasonal_naive_7d"] * 100
        )
        improvement_df = improvement.rename("lightgbm_mae_improvement_vs_naive_pct").reset_index()
        improvement_df.to_csv(output_dir / "lightgbm_vs_naive_improvement.csv", index=False)
